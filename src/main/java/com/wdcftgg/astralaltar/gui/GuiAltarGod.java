...
public class GuiAltarGod extends GuiInventoryContainerBase {

    private static final Random rand = new Random();
    public final ContainerAltarGod containerAltarBase;
    private static final BindableResource texAltarTrait = AssetLibrary.loadTexture(AssetLoader.TextureLocation.GUI, "guialtar5");
    private static final BindableResource texBlack = AssetLibrary.loadTexture(AssetLoader.TextureLocation.MISC, "black");

    public GuiAltarGod(InventoryPlayer playerInv, TileGodAltar tileAltar) {
        super(new ContainerAltarGod(playerInv, tileAltar), tileAltar);
        this.containerAltarBase = (ContainerAltarGod) super.inventorySlots;
    ...
    public Object findCraftableRecipe() {
        return this.findCraftableRecipe(false);
    }

    public Object findCraftableRecipe(boolean ignoreStarlightRequirement) {
        Object rec = AddedAltarRecipeRegistry.findMatchingRecipe(this.containerAltarBase.tileAltar, ignoreStarlightRequirement);
        if (rec != null) {
            if (rec instanceof IGatedRecipe) {
                return ((IGatedRecipe)rec).hasProgressionClient() ? rec : null;
            } else {
                return rec;
            }
        } else {
    ...
    protected void drawRect(int offsetX, int offsetY, int width, int height, double u, double v, double uLength, double vLength) {
        ...
        vb.pos((double)offsetX, (double)(offsetY + height), (double)this.zLevel).tex(u, v + vLength).endVertex();
        vb.pos((double)(offsetX + width), (double)(offsetY + height), (double)this.zLevel).tex(u + uLength, v + vLength).endVertex();
        vb.pos((double)(offsetX + width), (double)offsetY, (double)this.zLevel).tex(u + uLength, v).endVertex();
        vb.pos((double)offsetX, (double)offsetY, (double)this.zLevel).tex(u, v).endVertex();
        tes.draw();
    }
    ...
    protected void drawRect(int offsetX, int offsetY, int width, int height) {
        ...
        BufferBuilder vb = tes.getBuffer();
        vb.begin(7, DefaultVertexFormats.POSITION_TEX);
        vb.pos((double)offsetX, (double)(offsetY + height), (double)this.zLevel).tex(0.0, 1.0).endVertex();
        vb.pos((double)(offsetX + width), (double)(offsetY + height), (double)this.zLevel).tex(1.0, 1.0).endVertex();
        vb.pos((double)(offsetX + width), (double)offsetY, (double)this.zLevel).tex(1.0, 0.0).endVertex();
        vb.pos((double)offsetX, (double)offsetY, (double)this.zLevel).tex(0.0, 0.0).endVertex();
        tes.draw();
    }

    @Override
    public void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        Object o = findCraftableRecipe();
        ItemStack out = ItemStack.EMPTY;
        if (o instanceof AbstractAltarRecipe) {
            AbstractAltarRecipe rec = (AbstractAltarRecipe)  o;
            out = rec.getOutputForRender();
        } else if (o instanceof AddedAbstractAltarRecipe) {
            AddedAbstractAltarRecipe rec = (AddedAbstractAltarRecipe)  o;
            out = rec.getOutputForRender();
        }
        ...
        if(c != null && containerAltarBase.tileAltar.getMultiblockState() && ResearchManager.clientProgress.hasConstellationDiscovered(c.getUnlocalizedName())) {
            ...
            RenderConstellation.renderConstellationIntoGUI(c, 16, 59, zLevel, 58, 58, 2, new RenderConstellation.BrightnessFunction() {
                @Override
    ...
    public void renderGuiBackground(float partialTicks, int mouseX, int mouseY) {
        ...
        if(percFilled > 0) {
            ...
            drawRect(guiLeft + 29, guiTop + 140, (int) (232 * percFilled), 10,
                    uvOffset.key, uvOffset.value,
                    spriteStarlight.getULength() * percFilled, spriteStarlight.getVLength());

            Object o = findCraftableRecipe(true);
            boolean isNull = false;
            int req1 = -111;
//            AddedAbstractAltarRecipe aar = findCraftableRecipe(true);
            if (o instanceof AbstractAltarRecipe) {
                AbstractAltarRecipe aar = (AbstractAltarRecipe) o;
                req1 = aar.getPassiveStarlightRequired();
            } else if (o instanceof AddedAbstractAltarRecipe){
                AddedAbstractAltarRecipe aar = (AddedAbstractAltarRecipe) o;
                req1 = aar.getPassiveStarlightRequired();
            }
            if(req1 != -111) {
//                int req = aar.getPassiveStarlightRequired();
            ...
            AddedActiveCraftingTask task = containerAltarBase.tileAltar.getAddedActiveCraftingTask();
            if(task != null && task.getState() != AddedActiveCraftingTask.CraftingState.ACTIVE) {
                ...
                if(has < req) {
                    ...
                    drawRect(guiLeft + 11 + from, guiTop + 140, to, 10,
                            uvOffset.key + spriteStarlight.getULength() * percFilled, uvOffset.value,
                            spriteStarlight.getULength() * percReq, spriteStarlight.getVLength());
