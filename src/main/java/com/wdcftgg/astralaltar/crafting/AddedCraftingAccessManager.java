...
public class AddedCraftingAccessManager {
    ...
    public static void compile() {
        AddedAltarRecipeRegistry.compileRecipes();
...
//        removeAll(GrindstoneRecipeRegistry.mtRecipes);
        for (TileGodAltar.AltarLevel al : TileGodAltar.AltarLevel.values()) {
            removeAll(AddedAltarRecipeRegistry.mtRecipes.get(al));
        }
...
//        GrindstoneRecipeRegistry.mtRecipes.clear();
        AddedAltarRecipeRegistry.mtRecipes.clear();
        AddedAltarRecipeRegistry.recipes.clear();
        ...
        for (TileGodAltar.AltarLevel al : TileGodAltar.AltarLevel.values()) {
            AddedAltarRecipeRegistry.mtRecipes.put(al, new LinkedList<>());
            AddedAltarRecipeRegistry.recipes.put(al, new LinkedList<>());
        }

        //Loading default configurations how it'd be without Minetweaker
//        InfusionRecipeRegistry.loadFromFallback();
        AddedAltarRecipeRegistry.loadFromFallback();
...
//        GrindstoneRecipeRegistry.loadFromFallback();
    ...
    public static void registerMTAltarRecipe(AddedAbstractAltarRecipe recipe) {
        tryRemoveAltarRecipe(recipe.getNativeRecipe().getRegistryName());

        TileGodAltar.AltarLevel al = recipe.getNeededLevel();
        AddedAltarRecipeRegistry.mtRecipes.get(al).add(recipe);
        addRecipe(recipe);
    }
    ...
    @Deprecated
    public static void tryRemoveAltarRecipeByOutputAndLevel(ItemStack output, TileGodAltar.AltarLevel altarLevel) {
        markForRemoval(AddedAltarRecipeRegistry.removeFindRecipeByOutputAndLevel(output, altarLevel));
    }

    public static boolean tryRemoveAltarRecipe(ResourceLocation recipeRegistryName) {
        AddedAbstractAltarRecipe recipe = AddedAltarRecipeRegistry.getRecipeSlow(recipeRegistryName);
        recipe = AddedAltarRecipeRegistry.removeRecipeFromCache(recipe);
